	...
	if (\Storage::exists($filepath . $thumbFilename)) {
		...
		try {
			...
			if (in_array($type, ['logo'])) {
				...
				$image = \Image::make(\Storage::get($pathFromDb))->resize($width, $height, function ($constraint) use ($ratio, $upSize) {
					if ($ratio == '1') {
				...
				if ($resizeType == '0') {
					...
					// Fit
					$image = \Image::make(\Storage::get($pathFromDb))->fit($width, $height, function ($constraint) use ($ratio, $upSize) {
						if ($ratio == '1') {
							$constraint->aspectRatio();
						}
						if ($upSize == '1') {
 ...
 * @return bool
 */
function removeMatchedFilesRecursive($path, $pattern)
{
	if (is_file($path)) {
		...
		*/
		...
		if (!empty($files)) {
			foreach ($files as $file) {
				removeMatchedFilesRecursive($file, $pattern);
			}
		}
 ...
 */
function removeUnwantedFiles($path, $filenames = [])
{
	if (empty($filenames)) {
