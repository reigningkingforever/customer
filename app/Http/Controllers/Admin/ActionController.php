...
class ActionController extends Controller
{
	...
	public function clearImagesThumbnails()
	{
		...
		// Get the upload path
		$uploadBasePath = config('filesystems.disks.' . config('filesystems.default') . '.root') . DIRECTORY_SEPARATOR;
		$uploadPaths = [
			$uploadBasePath . 'files' . DIRECTORY_SEPARATOR,    // New path
			$uploadBasePath . 'pictures' . DIRECTORY_SEPARATOR, // Old path
		];
		...
		foreach ($uploadPaths as $uploadPath) {
			...
			try {
				...
				removeMatchedFilesRecursive($uploadPath, $pattern);
			} catch (\Exception $e) {
				Alert::error($e->getMessage())->flash();
				$errorFound = true;
				break;
			...
			try {
				...
				if (!file_exists($uploadPath . '.gitignore')) {
					$content = '*' . "\n"
						. '!.gitignore' . "\n";
					File::put($uploadPath . '.gitignore', $content);
				}
				
				// Removing all empty sub directories
				removeEmptySubDirs($uploadPath);
			} catch (\Exception $e) {
				Alert::error($e->getMessage())->flash();
				$errorFound = true;
				break;
